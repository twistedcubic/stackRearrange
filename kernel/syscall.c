...
fetchint(struct proc *p, uint addr, int *ip)
{
  ...
  if(p->pid != 1 && (addr < PGSIZE || (addr>p->sz && addr<p->stacksz) || addr>USERTOP || (addr+4>p->sz && addr+4 < p->stacksz) || addr+4>USERTOP))   
    return -1;
  *ip = *(int*)(addr);
  return 0;
}
...
fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  cprintf("stacksz: %p\n", p->stacksz); /////////////debug
  //  if(addr >= p->sz)
  if(p->pid != 1 && (addr < PGSIZE || (addr>p->sz && addr < p->stacksz) || addr>USERTOP))
	return -1;
  *pp = (char*)addr;
  //ep = (char*)p->sz;////
  for(s = *pp; s < (char*)USERTOP || (s<(char*)(p->sz) && s>(char*)PGSIZE); s++)
    if(*s == 0)
      return s - *pp;
  return -1;
}
